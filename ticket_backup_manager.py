...
class TicketBackupManager:
    ...
    def backup_closed_tickets(self, limit: int = 0) -> Dict:
        ...
        try:
            ...
            # ── Mirror run row to the first active tenant DB ─────────────
            # Only mirror when running in single-tenant/legacy mode (no
            # set_current_tenant).  In multi-tenant mode the row is already
            # written to the correct tenant DB, so mirroring would duplicate.
